 ...
 * @returns {Data}
 */
function visitValue<T>(
  value: T,
  refToRefIdentifierMap: Map<{}, string>
): MarshalledDataResult {
  // Check for ref hits
  ...
  switch (typeof value) {
    case "function":
      throw new TypeError(
        `Cannot marshal functions since this is considered to be a security risk!`
      );
      ...
      if (value === null) {
        ...
        return {
          ...
          value: <IMarshalledArrayData>(
            visitValue([...value], refToRefIdentifierMap)
          )
        };
        ...
        throw new TypeError(
          `WeakMaps cannot be marshalled since they aren't iterable`
        );
      } else if (value instanceof WeakSet) {
        throw new TypeError(
          `WeakSets cannot be marshalled since they aren't iterable`
        );
      } else {
        throw new TypeError(
          `Could not marshall a value of type: ${value.constructor.name}`
        );
      }
    }
  }
...
export function demarshall<T>(value: string | MarshalledData): T {
  const parsed: MarshalledDataResult = isMarshalledData(value)
    ? value
    : JSON.parse(value);
 ...
 * @returns {*}
 */
function demarshallValue(
  data: MarshalledDataResult,
  refMap: Map<string, {}>
): any {
  if (isJsonType(data)) {
    return data;
  ...
  if (isMarshalledData(data)) {
    switch (data[marshalledDataTypeKey]) {
      ...
      case "float64array":
        ...
        const newArray = getTypedArray(
          data[marshalledDataTypeKey],
          value.map(v => demarshallValue(v, refMap))
        );
        refMap.set(refMatch!, newArray);
        return newArray;
    ...
    return Object.assign(
      ...
      ...Object.entries(data).map(([key, objectValue]) =>
        key === marshalledDataTypeKey
          ? {}
          : { [key]: demarshallValue(<any>objectValue, refMap) }
      )
    );
  } else {
...
function isJsonType(data: any): data is JsonType {
  ...
  return (
    typeofData === "string" ||
    (typeofData === "number" && !isNaN(data) && data !== Infinity) ||
    typeofData === "boolean"
  );
}
...
function getTypedArray(
  ...
  switch (type) {
    ...
    default:
      throw new TypeError(
        `The given data type: ${type} can not be used with TypedArrays!`
      );
  }
}
